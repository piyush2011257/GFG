...
void cut_edge ( head h[], int i, int *dfsnum, int *low, int num, int *aprent );
...
{	int n, e, u, v;
	...
	cut_edge(h,0,dfsnum,low,0,parent);
	printf("\n");
	return 0;
}

void cut_edge ( head h[], int i, int *dfsnum, int *low, int num, int *parent )
{ 	dfsnum[i]=low[i]=num;
	printf("%d ", i);
	node *tmp= h[i].lnext;
	int child=0;
	...
	{	if ( dfsnum[tmp->data] == -1 )
		{	parent[tmp->data]=i;
			child++;
			cut_edge(h, tmp->data,dfsnum, low, num+1, parent);
